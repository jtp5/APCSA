...
public class Picture extends SimplePicture {
	...
	/** Method that turns the image into grey scale */
	...
	/** Method that fixes the fish underwater */
	...
	/** Method that mirrors vertically from right to left */
	...
	/** Method that mirrors horizontally from bot to top */
	public void mirrorHorizontallyBotToTop() {
		Pixel[][] pixels = this.getPixels2D();
		Pixel upperPixel = null;
		Pixel lowerPixel = null;
		int length = pixels.length;
	...
	public void mirrorDiagonaly() {
		...
		for (int row = 0; row < line; row++) {
			...
			if (line < pixels.length && line < pixels[0].length) {
				line++;
			}
		}
	}
	...
	public void copy2(Picture fromPic, int startRow, int startCol, int startRowFrom, int startColFrom, int endRowFrom, int endColFrom) {
		Pixel fromPixel = null;
		Pixel toPixel = null;
		Pixel[][] toPixels = this.getPixels2D();
		Pixel[][] fromPixels = fromPic.getPixels2D();
		for (int fromRow = startRowFrom, toRow = startRow; fromRow < endRowFrom
				&& toRow < toPixels.length; fromRow++, toRow++) {
			for (int fromCol = startColFrom, toCol = startCol; fromCol < endColFrom
					&& toCol < toPixels[0].length; fromCol++, toCol++) {
				fromPixel = fromPixels[fromRow][fromCol];
				toPixel = toPixels[toRow][toCol];
				toPixel.setColor(fromPixel.getColor());
	...
	public void edgeDetection(int edgeDist) {
		...
		for (int row = 0; row < pixels.length - 1; row++) {
			for (int col = 0; col < pixels[0].length; col++) {
				...
				if(upperPixel.colorDistance(lowerColor) > edgeDist) {
